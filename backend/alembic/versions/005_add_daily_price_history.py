...
def upgrade() -> None:
    ...
    if "daily_price_history" not in inspector.get_table_names():
        op.create_table(
            ...
            sa.UniqueConstraint(
                "ticker", "date", name="uq_daily_price_history_ticker_date"
            ),
        )
        op.create_index(
            "ix_daily_price_history_ticker_date",
            "daily_price_history",
            ["ticker", "date"],
        )
...
def downgrade() -> None:
    op.drop_index(
        "ix_daily_price_history_ticker_date", table_name="daily_price_history"
    )
