...
def _enrich_all(db, job: NgxJobLog) -> None:
    """Enrich all tickers with per-ticker data. Skips tickers refreshed within ENRICH_MAX_AGE."""
    cutoff = datetime.now(timezone.utc) - timedelta(seconds=ENRICH_MAX_AGE)
    rows = db.query(NgxTickerCache.ticker, NgxTickerCache.fundamentals_updated_at).all()
    to_enrich = [
        t for t, ts in rows
        if ts is None or (ts.replace(tzinfo=timezone.utc) if ts.tzinfo is None else ts) < cutoff
    ]
