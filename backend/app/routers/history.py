...
def price_history(
    ...
    try:
        rows = get_price_history(
            db, ticker=ticker.upper(), days=days, user_id=current_user.id
        )
        points = [
            PricePoint(
                ts=r["ts"],
...
def correlation(
    ...
    try:
        ...
        return CorrelationResponse(
            tickers=result["tickers"], matrix=result["matrix"], days=days
        )
    except Exception:
        log.exception("Error computing correlation matrix")
        raise HTTPException(
            status_code=500, detail="Failed to compute correlation matrix"
        )
...
def analytics(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="Failed to compute portfolio analytics"
        )
...
def relative_strength(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="Failed to compute relative strength"
        )
