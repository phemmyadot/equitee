...
class Holding(Base):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
    user_id: Mapped[int] = mapped_column(
        Integer, ForeignKey("users.id"), nullable=False, server_default="1"
    )
...
class PriceHistory(Base):
    ...
    snapshot: Mapped["PortfolioSnapshot"] = relationship(
        "PortfolioSnapshot", back_populates="prices"
    )
...
class DividendCache(Base):
    ...
    __table_args__ = (Index("ix_dividend_cache_ticker", "ticker"),)
...
class NgxTickerCache(Base):
    ...
    # Extended metrics (from statistics page + calculated)
    ...
    fcf_per_share: Mapped[Optional[float]] = mapped_column(Float, nullable=True)
    ...
    fcf_growth_yoy: Mapped[Optional[float]] = mapped_column(Float, nullable=True)
    dividend_growth_yoy: Mapped[Optional[float]] = mapped_column(Float, nullable=True)
    ...
    ev_fcf: Mapped[Optional[float]] = mapped_column(Float, nullable=True)
    ...
    dividend_updated_at: Mapped[Optional[datetime]] = mapped_column(DateTime(timezone=True), nullable=True)
