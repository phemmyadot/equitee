...
export function computeTargets(
  ...
  const direction: TargetResult['direction'] =
    signalTotal != null && signalTotal > 1
      ? 'buy'
      : signalTotal != null && signalTotal < -1
        ? 'sell'
        : 'hold';
  ...
  if (graham && graham > 0) {
    if (price < graham * 0.75) {
      const techFloor = tech ? Math.min(price * 0.9, tech * 0.97) : price * 0.9;
      const capHigh = w52h ? Math.min(price * 1.15, w52h * 1.02) : price * 1.15;
      buy_low = r2(Math.max(techFloor, price * 0.82));
      buy_high = r2(capHigh);
      rationale.push(
        `Trading at deep discount to Graham Number (₦${graham.toFixed(0)}) — in buy zone now`,
      );
      ...
      rationale.push(
        `Trading above fair value (Graham ₦${graham.toFixed(0)}) — buy on pullback only`,
      );
    }
  } else if (tech) {
  ...
  if (graham && graham > 0) {
    if (price < graham * 0.75) {
      sell_low = r2(Math.min(graham * 0.7, price * 1.4));
      sell_high = r2(Math.min(graham * 0.9, price * 2.5));
      ...
      sell_low = r2(floor);
      sell_high = r2(Math.max(graham * 1.2, w52h ? w52h * 1.05 : price * 1.4));
      rationale.push(`Sell zone at fair value / Graham premium (₦${graham.toFixed(0)})`);
    } else {
      sell_low = r2(Math.max(graham * 1.1, w52h ? w52h * 0.92 : price * 1.15));
      sell_high = r2(Math.max(graham * 1.4, w52h ? w52h * 1.05 : price * 1.4));
      rationale.push(`Sell zone at Graham premium (₦${(graham * 1.1).toFixed(0)}+)`);
    }
  } else if (w52h) {
    sell_low = r2(w52h * 0.9);
    sell_high = r2(w52h * 1.05);
    rationale.push(`Sell zone at 52W high resistance (₦${w52h.toFixed(0)})`);
  } else {
    sell_low = r2(price * 1.2);
    sell_high = r2(price * 1.4);
