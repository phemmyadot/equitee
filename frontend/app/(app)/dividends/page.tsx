  ...
  return (
    <div className="flex flex-col gap-0.5 min-w-[90px]">
      ...
      {value ? (
        <>
          <span
            className={`font-mono text-[11px] font-medium ${urgent ? 'text-[var(--warn)]' : past ? 'text-[var(--ink-4)] line-through' : 'text-[var(--ink-2)]'}`}
          >
            {value}
          </span>
          {days != null && !past && (
            <span
              className={`text-[9px] font-semibold ${urgent ? 'text-[var(--warn)]' : 'text-[var(--ink-4)]'}`}
            >
...
function DividendCard({ h, sector }: { h: DividendHolding; sector?: string }) {
  ...
  return (
    <div
      className={`card px-5 py-4 transition-shadow duration-150 hover:shadow-[var(--shadow-hover)] ${!hasDividend ? 'opacity-60' : ''}`}
    >
      {/* ── Header row ── */}
              ...
              <div key={h.ticker} className="flex items-center gap-4 py-2.5">
                ...
                <div className="flex-1 flex items-center justify-between gap-3 min-w-0">
                  <div className="flex items-center gap-2 min-w-0">
                    ...
                    <span className="text-[11px] text-[var(--ink-3)] truncate hidden sm:block">
                      {h.name}
                    </span>
              ...
              <div className="grid grid-cols-3 gap-3">
                {(
                  ...
                  <div key={label} className="bg-[var(--sidebar)] rounded-xl px-4 py-3">
                    <p className="text-[9px] font-bold uppercase tracking-widest text-[var(--ink-4)]">
                      {label}
                    </p>
