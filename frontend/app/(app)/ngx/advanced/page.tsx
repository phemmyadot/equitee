...
export default function NGXAdvancedPage() {
  ...
  const hhi =
    active.reduce((sum, s) => sum + ((s.CurrentEquity ?? 0) / totalEquity) ** 2, 0) * 10000;
  ...
  const mvSplit = useMemo(() => {
    ...
    const rows = active.map((s) => {
      ...
      const label =
        isMomentum && isValue ? 'Quality' : isMomentum ? 'Momentum' : isValue ? 'Value' : 'Neutral';
      return {
        ticker: s.Ticker,
        name: s.Stock,
        sector: s.Sector,
  ...
  const sectorFundamentals = useMemo(() => {
    ...
    const sectorAvgs: Record<
      string,
      { avg_pe: number | null; avg_roe: number | null; count: number }
    > = {};
  ...
  const sectorBubble = {
    ...
    marker: {
      ...
      color: sectors.map((s) => sectorColor(s.Sector)),
      opacity: 0.8,
      line: { color: '#fff', width: 1.5 },
    },
  ...
  return (
    <div className="space-y-5">
        ...
        <div className="flex items-center justify-between mb-3">
          ...
          {wPE != null &&
            (() => {
              ...
              const msg =
                below.length === PE_BENCHMARKS.length
        ...
        <div className="relative h-8 mt-4 mb-6">
          ...
          {PE_BENCHMARKS.map((b) => {
            ...
            return (
              ...
              >
                ...
                <span
                  className="text-[8px] font-bold mt-1 whitespace-nowrap"
                  style={{ color: b.color }}
                >
                  {b.label} {b.value}×
                </span>
          ...
          {wPE != null && wPE > 0 && wPE <= PE_MAX && (
            <div
              ...
              style={{ left: `${(wPE / PE_MAX) * 100}%`, transform: 'translateX(-50%)' }}
            >
              <div
                className="w-3 h-3 rounded-full border-2 border-white shadow-md"
                style={{ background: 'var(--accent)', marginTop: '0px' }}
              />
              <span className="text-[8px] font-bold text-[var(--accent)] mt-1 whitespace-nowrap">
                You {wPE.toFixed(1)}×
              </span>
            </div>
          )}
      ...
      {mvSplit && (
        <div className="bg-white rounded-2xl border border-[var(--border)] px-5 py-4">
          <p className="text-[10px] font-bold uppercase tracking-widest text-[var(--ink-4)] mb-3">
            Momentum vs Value
          </p>
          <div className="flex gap-4 flex-wrap mb-3">
            {(
              [
                ['Momentum', '#10B981', 'ROE>15% + Return>0%'],
                ['Value', '#6366F1', 'P/E<12 + undervalued'],
                ['Quality', '#F59E0B', 'High ROE + Low P/E'],
                ['Neutral', '#9CA3AF', 'No clear signal'],
              ...
              <thead>
                <tr className="border-b border-[var(--border)]">
                  {['Ticker', 'Type', 'Return', 'P/E', 'ROE %'].map((h) => (
                    <th
                      key={h}
                      className={`py-1.5 px-2 text-[9px] font-semibold uppercase tracking-wide text-[var(--ink-4)] ${h === 'Ticker' || h === 'Type' ? 'text-left' : 'text-right'}`}
                    >
                      {h}
                    </th>
                  ))}
              ...
              <tbody>
                {mvSplit.rows.map((r) => {
                  ...
                  return (
                    <tr
                      key={r.ticker}
                      className="border-b border-[var(--border)] hover:bg-[var(--sidebar)]"
                    >
                      <td className="py-1.5 px-2 font-mono font-semibold text-[var(--ink)]">
                        {r.ticker}
                      </td>
                      ...
                      <td
                        className={`py-1.5 px-2 font-mono text-right text-[11px] ${r.ret >= 0 ? 'text-[var(--gain)]' : 'text-[var(--loss)]'}`}
                      >
                        {r.ret >= 0 ? '+' : ''}
                        {r.ret.toFixed(1)}%
                      </td>
                      <td className="py-1.5 px-2 font-mono text-right text-[var(--ink-2)]">
                    ...
                    return [
                        ...
                        >
                          ...
                          >
                            ...
                            <span className="ml-2 font-normal text-[var(--ink-4)]">
                              avg P/E {fmt(sa.avg_pe)} · ROE {fmt(sa.avg_roe)}%
                            </span>
                          </td>
                        </tr>
                      ),
                      ...
                      >
                        ...
                        <td className="text-right py-2 px-3 font-mono">
                          <span className="text-[var(--ink-2)]">{fmt(r.pe)}</span>
                          {r.pe_rank != null && (
                            <span className={`ml-1 text-[9px] ${rankColor(r.pe_rank)}`}>
                              {r.pe_rank}p
                            </span>
                          )}
                        </td>
                        <td className="text-right py-2 px-3 font-mono">
                          <span className="text-[var(--ink-2)]">{fmt(r.roe)}</span>
                          {r.roe_rank != null && (
                            <span className={`ml-1 text-[9px] ${rankColor(r.roe_rank)}`}>
                              {r.roe_rank}p
                            </span>
                          )}
                        </td>
