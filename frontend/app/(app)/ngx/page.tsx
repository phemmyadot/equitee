...
function toPerformance(row: StockRow) {
  return {
    beta: row.Beta ?? null,
    ma_50: row.Ma50 ?? null,
    ma_200: row.Ma200 ?? null,
    week_52_high: row.Week52High ?? null,
    week_52_low: row.Week52Low ?? null,
...
export default function NGXOverviewPage() {
  ...
  const active = holdings.filter((s) => s.CurrentEquity != null);
  const k = resp?.kpis;
  const ck = resp?.combined_kpis;
  ...
  const sectorGainBar = {
    ...
    y: sectors.map((s) => s.GainPct),
    marker: {
      color: sectors.map((s) => (s.GainPct >= 0 ? sectorColor(s.Sector) : COLORS.loss)),
      opacity: 0.85,
    },
    hovertemplate: '<b>%{x}</b><br>%{y:.1f}%<extra></extra>',
  };
