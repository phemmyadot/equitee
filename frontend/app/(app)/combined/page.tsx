...
export default function CombinedPage() {
  ...
  const ngxReturnBar = {
    ...
    marker: {
      color: ngx_stocks.map((s) => ((s.ReturnPct ?? 0) >= 0 ? COLORS.accent : COLORS.loss)),
      opacity: 0.85,
    },
  ...
  const usReturnBar = {
    ...
    marker: {
      color: us_stocks.map((s) => ((s.ReturnPct ?? 0) >= 0 ? COLORS.teal : COLORS.loss)),
      opacity: 0.85,
    },
  ...
  return (
    <div className="space-y-5">
        ...
        {isFirstLoad ? (
          ...
          <>
            ...
            <KPICard
              ...
              accent={isPositive(ngxRetPct) ? 'gain' : 'loss'}
              delay={150}
            />
            <KPICard
              label="US Return"
              ...
              accent={isPositive(usRetPct) ? 'gain' : 'loss'}
              delay={200}
            />
          </>
        )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <ChartCard
          title="NGX vs US Split"
          subtitle="by USD value"
          loading={isFirstLoad}
          height={320}
        >
          <PlotlyChart
      ...
      <ChartCard
        title="Combined Sector Breakdown"
        subtitle="NGX + US converted to USD"
        loading={isFirstLoad}
        height={340}
      >
        <PlotlyChart
