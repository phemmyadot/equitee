...
function Btn({
  ...
  const base =
    'inline-flex items-center gap-1.5 font-semibold rounded-md transition-colors duration-150 disabled:opacity-50 disabled:cursor-not-allowed whitespace-nowrap';
  const sizes = { sm: 'px-3 py-1.5 text-[11px]', xs: 'px-2 py-1 text-[10px]' };
  const variants = {
    primary: 'bg-[var(--accent)] text-white hover:bg-[#17A06B]',
    secondary:
      'bg-white border border-[var(--border)] text-[var(--ink-2)] hover:border-[var(--border-strong)] hover:text-[var(--ink)]',
...
export default function SettingsPage() {
  ...
  const active = holdings.filter((h) => h.is_active && (filter === 'all' || h.market === filter));

  const fmtCost = (h: HoldingRecord) =>
    h.market === 'ngx' ? fmtNGN(h.avg_cost) : fmtUSD(h.avg_cost);
  const fmtValue = (h: HoldingRecord) =>
    h.market === 'ngx' ? fmtNGN(h.shares * h.avg_cost) : fmtUSD(h.shares * h.avg_cost);
                ...
                {active.map((h) => (
                  <tr key={h.id}>
                    ...
                    <td className="right font-mono text-[var(--ink)]">
                      {h.shares.toLocaleString(undefined, { maximumFractionDigits: 6 })}
                    </td>
                    ...
                    <td className="right">
                      <div className="flex items-center justify-end gap-1">
                        <Btn
                          size="xs"
                          variant="ghost"
                          onClick={() => setModal({ type: 'buy', holding: h })}
                          title="Buy more"
                        >
                          <IconPlus width={11} height={11} />
                          Buy
                        ...
                        <Btn
                          size="xs"
                          variant="ghost"
                          onClick={() => setModal({ type: 'sell', holding: h })}
                          title="Sell shares"
                        >
                        ...
                        <Btn
                          size="xs"
                          variant="danger"
                          onClick={() => setModal({ type: 'delete', holding: h })}
                          title="Delete"
                        >
              ...
              <tbody>
                ...
                {closed.map((c) => (
                  <tr key={c.id}>
                    ...
                    <td>
                      <span className="badge badge-nodata">{c.market.toUpperCase()}</span>
                    </td>
                    <td
                      className={`right font-mono font-semibold ${c.realized_pl >= 0 ? 'text-[var(--gain)]' : 'text-[var(--loss)]'}`}
                    >
                      {c.market === 'ngx' ? fmtNGN(c.realized_pl) : fmtUSD(c.realized_pl)}
                    </td>
                    <td className="text-[var(--ink-4)] font-mono text-[11px]">
                      {new Date(c.closed_at).toLocaleDateString(undefined, {
                        year: 'numeric',
                        month: 'short',
                        day: 'numeric',
                      })}
      ...
      {modal?.type === 'delete' && (
        <Modal title={`Delete ${modal.holding.ticker}?`} onClose={() => setModal(null)}>
          <p className="text-[12px] text-[var(--ink-3)]">
            ...
            <strong className="text-[var(--ink)]">{modal.holding.name}</strong> and all its data.
    ...
    <Modal title={`Buy More · ${holding.ticker}`} onClose={onClose}>
      <div className="bg-[var(--canvas)] rounded-lg px-3 py-2.5 text-[11px] font-mono space-y-0.5">
        <div className="flex justify-between">
          <span className="text-[var(--ink-4)]">Current shares</span>
          <span>{holding.shares.toLocaleString(undefined, { maximumFractionDigits: 6 })}</span>
        </div>
        <div className="flex justify-between">
          <span className="text-[var(--ink-4)]">Current avg cost</span>
          <span>
            {cur}
      ...
      {newAvg !== null && (
        <div className="bg-[var(--accent-light)] rounded-md px-3 py-2 text-[11px] font-mono flex justify-between">
          ...
          <span className="font-semibold text-[var(--accent)]">
            {cur}
            {newAvg.toLocaleString(undefined, { maximumFractionDigits: 4 })}
          </span>
        </div>
      )}
      {error && <p className="text-[11px] text-[var(--loss)]">{error}</p>}
...
function SellModal({
  ...
  const submit = async () => {
    ...
    if (sharesNum > holding.shares) {
      setError(`Max ${holding.shares} shares`);
      return;
    }
    setBusy(true);
    try {
  ...
  return (
    <Modal title={`Sell · ${holding.ticker}`} onClose={onClose}>
      <div className="bg-[var(--canvas)] rounded-lg px-3 py-2.5 text-[11px] font-mono space-y-0.5">
        <div className="flex justify-between">
          <span className="text-[var(--ink-4)]">Shares held</span>
          <span>{holding.shares.toLocaleString(undefined, { maximumFractionDigits: 6 })}</span>
        </div>
        <div className="flex justify-between">
          <span className="text-[var(--ink-4)]">Avg cost</span>
          <span>
            {cur}
            {holding.avg_cost.toLocaleString(undefined, { maximumFractionDigits: 4 })}
          </span>
