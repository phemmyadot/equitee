...
import { plotlyLayout, COLORS } from '@/utils/theme';
import { fmtUSD, fmtPct, isPositive } from '@/utils/formatters';
  ...
  return (
    <div className="space-y-5">
      ...
      {!loading && !hasData && (
        <div className="card px-6 py-12 flex flex-col items-center gap-3 text-center">
          ...
          <p className="text-[11px] text-[var(--ink-4)] max-w-[320px]">
            Snapshots are written each time{' '}
            <code className="font-mono bg-[var(--canvas)] px-1 rounded">/api/data</code> is called.
            Refresh the dashboard a few times and come back — data will appear here automatically.
          </p>
        </div>
      )}
      ...
      {(loading || hasData) && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <ChartCard
            title="NGX Unrealized G/L Over Time"
            subtitle="in ₦"
            loading={loading}
            height={260}
          >
