...
export function ErrorMessage({ message }: { message: string }) {
  return (
    ...
    "
    >
      <IconAlertCircle width={14} height={14} className="shrink-0 mt-0.5" />
      <span>{message}</span>
    </div>
  );
}
  ...
  return (
    <div className="price-banner flex flex-wrap items-center gap-x-5 gap-y-1">
      <span className="flex items-center gap-1.5">
        ...
        <strong className="text-[var(--ink-2)] font-semibold">
          {live}/{total}
        </strong>
      ...
      {age !== null && <span className="text-[var(--ink-4)]">cached {age}m ago</span>}
    </div>
  );
}
