...
import { fmtNGNFull } from '@/lib/formatters';
...
interface Props {
  ...
  w52l?:      number | null;
  w52h?:      number | null;
  label:      string;   // signal label e.g. "Buy", "Hold"
  color:      string;   // signal colour CSS var
}
...
export default function PriceTargets({ result, price, w52l, w52h, label, color }: Props) {
  ...
  // ── Price band visual ──────────────────────────────────────────────────
  // Range: from min(buy_low, w52l, price*0.7) to max(sell_high, w52h, price*1.4)
  ...
  function pct(v: number) { return ((v - bandMin) / span * 100).toFixed(2) + '%'; }
  ...
  const dirLabel =
    direction === 'buy'  ? 'Buy zone' :
    direction === 'sell' ? 'Sell zone' :
    'Hold — monitor zones';

  const dirColor =
    direction === 'buy'  ? 'var(--gain)' :
    direction === 'sell' ? 'var(--loss)' :
    'var(--ink-4)';
  ...
  return (
    <div className="card px-5 py-5">
      ...
      <div className="flex justify-between text-[9px] text-[var(--ink-4)] mb-4">
        ...
        <span className="font-semibold" style={{ color }}>▲ {fmtNGNFull(price)}</span>
        <span>{w52h ? fmtNGNFull(w52h) : ''}</span>
      </div>

      {/* Buy / Sell zone figures */}
      <div className="grid grid-cols-2 gap-3 mb-4">
        <div className="rounded-lg p-3 border border-[var(--border)]"
