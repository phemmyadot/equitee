 ...
 *   RSI_ERROR_THRESHOLD = 100.01 — RSI > 100 is a data error; exclude from composite
...
const RSI_ERROR_THRESHOLD = 100.01;
    ...
    dimensions: [
      {
        ...
        weight: WEIGHTS.mom,
        score: mom.score,
        contrib: mom.score * WEIGHTS.mom * 10,
        signals: mom.signals,
      },
      {
        name: 'Valuation',
        weight: WEIGHTS.val,
        score: val.score,
        contrib: val.score * WEIGHTS.val * 10,
        signals: val.signals,
      },
      {
        name: qualLabel,
      ...
      },
      {
        ...
        weight: WEIGHTS.div,
        score: div.score,
        contrib: div.score * WEIGHTS.div * 10,
        signals: div.signals,
      },
      {
        name: 'Risk',
        weight: WEIGHTS.risk,
        score: risk.score,
        contrib: risk.score * WEIGHTS.risk * 10,
...
export default function SignalScore({
  ...
  const cx = 64,
    cy = 64;
  const gaugeAngle = ((total + 10) / 20) * 180;
  ...
  return (
    <div className="card px-5 py-5">
      ...
      <div className="flex flex-col sm:flex-row gap-6 items-start">
        ...
        <div className="flex-1 space-y-2.5 w-full">
          {dimensions.map((dim) => {
            ...
            const dCol =
              dim.score > 0.3 ? 'var(--gain)' : dim.score < -0.3 ? 'var(--loss)' : 'var(--ink-4)';
            return (
              <div key={dim.name}>
                ...
                {dim.signals[0] && (
                  <p className="text-[9px] text-[var(--ink-4)] mt-0.5 leading-tight">
                    {dim.signals[0]}
                  </p>
                )}
              </div>
      ...
      <p className="mt-4 text-[9px] text-[var(--ink-4)] leading-relaxed border-t border-[var(--border)] pt-3">
        Signal score is a quantitative model based on available fundamental and technical data. It
        is not investment advice. Always conduct your own research before making any investment
        decision.
      </p>
    </div>
  );
